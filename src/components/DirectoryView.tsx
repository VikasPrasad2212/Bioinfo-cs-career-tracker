    ...
    <div className="space-y-6 pb-12">
      ...
      <div className="bg-emerald-950/40 border border-emerald-500/20 rounded-xl p-3.5 sm:p-4 flex flex-wrap items-center justify-between gap-3 text-xs">
          ...
          <div>
            ...
            <span className="text-slate-600">
              {liveSyncInfo?.lastUpdated
                ? `Last synchronized with NCBI / Europe PMC & LinkedIn on ${new Date(liveSyncInfo.lastUpdated).toLocaleDateString()} at ${new Date(liveSyncInfo.lastUpdated).toLocaleTimeString()}`
        ...
        <div className="flex items-center gap-2">
          <a
            href="https://www.linkedin.com/jobs/search/?keywords=bioinformatics+intern+india"
            target="_blank"
            rel="noreferrer"
            className="inline-flex items-center gap-1 px-2.5 py-1 rounded-lg bg-blue-100 hover:bg-blue-200 text-blue-800 font-semibold text-[11px] transition-colors"
          >
            <span>Live LinkedIn Openings</span>
      ...
      {/* Live Google Jobs / LinkedIn Listings Card (If synced by automated harvester) */}
      {liveSyncInfo?.liveJobsAndInternships && liveSyncInfo.liveJobsAndInternships.length > 0 && (
        <div className="bg-blue-50/70 rounded-xl p-4 border border-blue-200 space-y-3">
          <div className="flex items-center justify-between">
            <h3 className="font-bold text-xs sm:text-sm text-blue-950 flex items-center gap-1.5">
              <Zap className="w-4 h-4 text-blue-600" />
              <span>Real-Time Job Openings (Discovered via LinkedIn & Google Jobs)</span>
            </h3>
            <span className="text-[11px] font-semibold text-blue-700">
              {liveSyncInfo.liveJobsAndInternships.length} Live Openings Synced
            </span>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-2.5">
            {liveSyncInfo.liveJobsAndInternships.map((job) => (
              <div key={job.id} className="bg-white p-3 rounded-lg border border-blue-100 shadow-2xs flex items-center justify-between gap-3">
                <div className="space-y-0.5 min-w-0">
                  <h4 className="font-bold text-xs text-slate-900 truncate">{job.title}</h4>
                  <p className="text-[11px] text-slate-600 truncate">{job.company} • {job.location}</p>
                  <span className="text-[10px] text-blue-600 font-medium">{job.via}</span>
                </div>
                <a
                  href={job.applyUrl}
                  target="_blank"
                  rel="noreferrer"
                  className="px-2.5 py-1 bg-blue-600 hover:bg-blue-700 text-white rounded text-xs font-semibold shrink-0 flex items-center gap-1"
                >
                  <span>Apply</span>
                  <ExternalLink className="w-3 h-3" />
                </a>
      ...
      {/* Direct Company Career Portals & Startup Hub (PopVax, Strand, Elucidata, Pandorum, etc.) */}
      <div className="bg-slate-50 rounded-xl p-4 border border-slate-200/80 space-y-3">
        <div className="flex items-center justify-between flex-wrap gap-2">
          <div className="flex items-center gap-2">
            <Building2 className="w-4 h-4 text-indigo-600" />
            <h3 className="font-bold text-xs sm:text-sm text-slate-900">
              Fast-Track Direct Company Career Portals
            </h3>
          </div>
          <span className="text-[11px] text-slate-500 font-medium">
            Direct hiring portals for computational biology startups & biotech leaders
          </span>
        </div>

        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-2.5">
          {(liveSyncInfo?.companyCareerPortals || [
            { name: 'PopVax', focus: 'mRNA Vaccine Design & AI', location: 'Hyderabad / Remote', careerUrl: 'https://popvax.com/careers', tag: 'Deep Learning / Transformers', stipendRange: '₹35k - ₹50k/mo' },
            { name: 'Strand Life Sciences', focus: 'Clinical NGS & Software', location: 'Bengaluru', careerUrl: 'https://strandls.com/careers/', tag: 'Cloud / Distributed DBs', stipendRange: '₹30k - ₹40k/mo' },
            { name: 'Elucidata', focus: 'Biomedical Data Platform (Polly)', location: 'Delhi NCR / Remote', careerUrl: 'https://elucidata.io/careers', tag: 'Python / Data Eng', stipendRange: '₹35k - ₹45k/mo' },
            { name: 'MedGenome Labs', focus: 'High-Throughput Sequencing', location: 'Bengaluru', careerUrl: 'https://www.medgenome.com/careers/', tag: 'AWS / Nextflow', stipendRange: '₹25k - ₹35k/mo' },
            { name: 'Bugworks Research', focus: 'AI for Antibacterial Drug Design', location: 'Bengaluru', careerUrl: 'https://bugworksresearch.com/', tag: 'Cheminformatics / GNNs', stipendRange: '₹28k - ₹38k/mo' },
            { name: 'Pandorum Technologies', focus: 'Biomaterials & 3D Bioprinting', location: 'Bengaluru', careerUrl: 'https://pandorumtechnologies.com/careers/', tag: 'Vision / Modeling', stipendRange: '₹25k - ₹35k/mo' },
            { name: 'Premas Biotech', focus: 'Structural Protein Analytics', location: 'Gurugram', careerUrl: 'https://www.premasbiotech.com/careers/', tag: 'Predictive ML', stipendRange: '₹22k - ₹30k/mo' },
            { name: 'Mapmygenome', focus: 'Preventive Genomics Analytics', location: 'Hyderabad', careerUrl: 'https://mapmygenome.in/careers', tag: 'Full-Stack / Bio-Viz', stipendRange: '₹20k - ₹25k/mo' }
          ]).map((company, idx) => (
            <div key={idx} className="bg-white p-3 rounded-lg border border-slate-200/90 shadow-2xs flex flex-col justify-between hover:border-indigo-300 transition-colors">
              <div>
                <div className="flex items-center justify-between gap-1.5 mb-1">
                  <span className="font-bold text-xs text-slate-900 truncate">{company.name}</span>
                  <span className="text-[10px] font-bold px-1.5 py-0.5 rounded bg-emerald-50 text-emerald-700 shrink-0">
                    {company.stipendRange}
                  </span>
                </div>
                <p className="text-[11px] text-slate-600 line-clamp-1">{company.focus}</p>
                <div className="mt-1.5 flex items-center justify-between text-[10px] text-slate-500">
                  <span>{company.location}</span>
                  <span className="text-indigo-600 font-medium truncate max-w-[110px]">{company.tag}</span>
                </div>
              </div>
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="mt-2.5 w-full py-1 px-2 rounded bg-slate-100 hover:bg-indigo-600 hover:text-white text-slate-700 text-[11px] font-semibold flex items-center justify-center gap-1 transition-colors"
              >
